...
@Service("$assignRole")
public class OperationAssignRole extends AbstractOperationExecutor {

    @Autowired
    HttpServletRequest request;

    private Log log = LogFactory.getLog(OperationAssignRole.class);
    ...
    public Structure execute(Map<String, String[]> requestParameters, Map<String, String[]> headers, Structure input,
        ...
        if (id.equalsIgnoreCase("content")) {
            ...
            if (roles[0].equalsIgnoreCase("SupportRole")) {
                ...
                throw new FHIRResourceHandlingException("Unknown role", null);
            }
            return null;
        }
                ...
                if (scope.equals(deployment.getScope())) {
                    for (Role r : deployment.getRole()) {
                        if (r.getId() != null && r.getId().equalsIgnoreCase(roles[0])) {
                            foundRole = true;
                            resRepo.grantRole(scenario, r, users[0], initializeRequestContext(endpoint), null);
                        }
                    }
                }
        ...
        if (roles[0].equalsIgnoreCase("SupportRole")) {
            if (foundRole) {
                ...
                resRepo.grantRole(scenario, RoleCreator.getDefaultSupportRole(), users[0],
            ...
            if (foundRole) {
                ...
                resRepo.grantRole(scenario, RoleCreator.getSuperSupportRole(), users[0],
                        initializeRequestContext(endpoint), null);
            }
        } else {
            if (!foundRole) {
