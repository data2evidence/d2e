/**
    ...
    export function applyToUnquotedParts(inputSql, func) {
        let processedSql = "";
        const escapeRexExp = /[\'\"]/g;
        const quotationCharactersUsed = inputSql.match(escapeRexExp);
        if (quotationCharactersUsed) {
            const quotationSplitArray = inputSql.split(escapeRexExp);
            let i;
    ...
    export function uppercaseUnquotedParts(inputSql) {
        const uppercasedSql = applyToUnquotedParts(inputSql, function(str) {
    ...
    export function normalizeSql(rawSql) {
        const spacedParenthesesSql = spaceParantheses(rawSql);
        const evenSpacedSql = evenOutSpaces(spacedParenthesesSql);
        const normalizedSql = uppercaseUnquotedParts(evenSpacedSql);
    ...
    export function replaceNamedAliases(sqlStr, aliases) {
        ...
        const aliasRegExp = new RegExp(" \(" + aliases.join("|") + "\)\( |.|,|$\)",
            "g");
        ...
        const seenAliases = {};
        let aliasCounter = 1;
        const processedString = applyToUnquotedParts(sqlStr, function(argStr) {
            const processedArgStr = argStr.replace(aliasRegExp, function(fullMatch, capture1, capture2) {
                let result;
    ...
    export function findAliases(sqlStr) {
        ...
        // alias.
        const aliasRegExp = /(?: AS (\w+)(?: |,|$))|(?:(?:FROM|(?:JOIN(?: \\\()?)) (?:(?:\")?\w+(?:\")?\\\.)?(?:\")?(?:\w+|(?:\w+\\\.)*\w+::(?:\w+\\\.)*\w+)(?:\")?(?! (?:WHERE|GROUP|HAVING)) (\w+)(?:$| ))/g;
        let match;
        const aliases = [];
    ...
    export function replaceAliases(sqlStr) {
        const aliases = findAliases(sqlStr);
        const processedString = replaceNamedAliases(sqlStr, aliases);
        return processedString;
    }
    ...
    export function getSqlRegExpString(rawSqlString) {
        const sNormalizedSql = normalizeSql(rawSqlString);
        const sEscapedSqlString = escapeRegExp(sNormalizedSql);
        const sEscapedSqlRegExpString = replaceAliases(sEscapedSqlString);
    ...
    export function getSandwichedString(str, startStrings, endStrings) {
        const startRegExp = startStrings.join("|");
        let endRegExp = "$";
        if (endStrings.length > 0) {
            endRegExp += "|" + endStrings.join("|");
        }
        const re = new RegExp("((?:" + startRegExp + ")\\s*)(.+?)(?:\\s*(?:" + endRegExp + "))", "g");
        const sandwichStr = re.exec(str)[2];
        return sandwichStr;
    }
    ...
    export function expectEqualAsQueryObject(q1, q2) {
        const query1 = q1._prepareQuery();
        const query2 = q2._prepareQuery();
    ...
    export function createConfig({id, version, status, name, type, data, dependentConfigId, dependentConfigVersion, callback, connection}) {
    try {
        const query = `INSERT INTO "ConfigDbModels_Config"
        ("Id", "Version", "Status", "Name", "Type", "Creator", "Created", "Modifier", "Modified", "Data", "ParentId", "ParentVersion")
        ...
        const parameters = [
            ...
            { value: "SYSTEM" },
            { value: data },
            { value: dependentConfigId || "" },
            { value: dependentConfigVersion || "" }];
        connection.executeUpdate(query, parameters, (err, result) => {
            callback(err);
        });
    } catch (err) {
        callback(err);
    ...
    export function deleteAllConfigs({connection, callback}) {
    connection.executeUpdate(`delete from  "ConfigDbModels_Config"`, [], (err, data) => {
        callback(err);
    });
}
