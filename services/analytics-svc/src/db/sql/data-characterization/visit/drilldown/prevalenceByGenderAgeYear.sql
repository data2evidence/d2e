SELECT
  ...
  cast(
    CAST(
      CASE
        WHEN num_analysis_id = 204 THEN num_stratum_4
        ELSE null
      END AS INT
    ) * 10 AS VARCHAR(11)
  ) || '-' || cast(
  ...
  CAST(
    CASE
      WHEN num_analysis_id = 204 THEN num_stratum_2
      ELSE null
    END AS INT
  ) AS x_calendar_year,
  ROUND(
    1000 * (1.0 * num_count_value / denom_count_value),
    5
  ) AS y_prevalence_1000_pp
FROM
  (
    SELECT
  ...
  INNER JOIN @vocab_database_schema.concept c1 ON CAST(
    CASE
      WHEN num_analysis_id = 204 THEN num_stratum_1
      ELSE null
    END AS INT
  ) = c1.concept_id
  INNER JOIN @vocab_database_schema.concept c2 ON CAST(
    CASE
      WHEN num_analysis_id = 204 THEN num_stratum_3
      ELSE null
    END AS INT
  ) = c2.concept_id
WHERE
