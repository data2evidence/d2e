...
def create_secrets():
    ...
    Secret(value=os.getenv('IDP__ALP_DATA__CLIENT_SECRET')).save(name="idp-alp-data-client-secret", overwrite=True)
    Secret(value=os.getenv('MINIO__SECRET_KEY')).save(name="minio-secret-key", overwrite=True)
    Secret(value=os.getenv('STRATEGUS__KEYRING_PASSWORD')).save(name="strategus-keyring-password", overwrite=True)
...
def create_prefect_variables():
    ...
    Variable.set("minio_endpoint", os.getenv("MINIO__ENDPOINT"), overwrite=True)
    Variable.set("minio_port", os.getenv("MINIO__PORT"), overwrite=True)
    Variable.set("minio_access_key", os.getenv("MINIO__ACCESS_KEY"), overwrite=True)
    ...
    Variable.set("python_verify_ssl", os.getenv("PYTHON_VERIFY_SSL"), overwrite=True)
    ...
    # For strategus
    Variable.set("cohort_generator_module_settings_url", os.getenv("OHDSI__R_COHORT_GENERATOR_MODULE_SETTINGS_URL"), overwrite=True)
    Variable.set("cohort_diagnostics_module_settings_url", os.getenv("OHDSI__R_COHORT_DIAGNOSTICS_MODULE_SETTINGS_URL"), overwrite=True)
    ...
    if os.getenv("LIQUIBASE_PATH"):
        Variable.set("liquibase_path", os.getenv("LIQUIBASE_PATH"), overwrite=True)
        
    if os.getenv("HANA__DRIVER_CLASS_PATH"):
        Variable.set("hana_driver_class_path", os.getenv("HANA__DRIVER_CLASS_PATH"), overwrite=True)
        
    if  os.getenv("POSTGRES__DRIVER_CLASS_PATH"):
        Variable.set("postgres_driver_class_path", os.getenv("POSTGRES__DRIVER_CLASS_PATH"), overwrite=True)
