...
import { IDataQualityCheckResult } from '../types'
...
export class DataQualityOverviewParser {
  parse = (items: IDataQualityCheckResult[]) => {
    ...
    const verificationPlausibilityPass = items.filter(
      c =>
        (c.hasOwnProperty('passed') ? c.passed == 1 || c.notApplicable == 1 : c.failed == 0) &&
    ...
    const verificationPlausibilityFail = items.filter(
      c =>
        (c.hasOwnProperty('passed') ? c.failed == 1 || c.isError == 1 : c.failed == 1) &&
        c.context == 'Verification' &&
        c.category == 'Plausibility'
    ).length
    ...
    const verificationPlausibilityPercentPass =
      verificationPlausibilityTotal == 0
        ? '-'
        : Math.round((verificationPlausibilityPass / verificationPlausibilityTotal) * 100) + '%'
    ...
    const verificationConformancePercentPass =
      verificationConformanceTotal == 0
        ? '-'
        : Math.round((verificationConformancePass / verificationConformanceTotal) * 100) + '%'
    ...
    const VerificationCompletenessPercentPass =
      verificationCompletenessTotal == 0
        ? '-'
        : Math.round((verificationCompletenessPass / verificationCompletenessTotal) * 100) + '%'
    ...
    const verificationFail = items.filter(
      c => (c.hasOwnProperty('passed') ? c.failed == 1 || c.isError == 1 : c.failed == 1) && c.context == 'Verification'
    ).length

    const verificationTotal = items.filter(c => c.context == 'Verification').length

    const verificationPercentPass =
      verificationTotal == 0 ? '-' : Math.round((verificationPass / verificationTotal) * 100) + '%'
    ...
    const validationPlausibilityPercentPass =
      validationPlausibilityTotal == 0
        ? '-'
        : Math.round((validationPlausibilityPass / validationPlausibilityTotal) * 100) + '%'
    ...
    const validationConformancePercentPass =
      validationConformanceTotal == 0
        ? '-'
        : Math.round((validationConformancePass / validationConformanceTotal) * 100) + '%'
    ...
    const validationCompletenessPercentPass =
      validationCompletenessTotal == 0
        ? '-'
        : Math.round((validationCompletenessPass / validationCompletenessTotal) * 100) + '%'
    ...
    const validationFail = items.filter(
      c => (c.hasOwnProperty('passed') ? c.failed == 1 || c.isError == 1 : c.failed == 1) && c.context == 'Validation'
    ).length

    const validationTotal = items.filter(c => c.context == 'Validation').length

    const validationPercentPass =
      validationTotal == 0 ? '-' : Math.round((validationPass / validationTotal) * 100) + '%'
    ...
    const plausibilityFail = items.filter(
      c =>
        (c.hasOwnProperty('passed') ? c.failed == 1 || c.isError == 1 : c.failed == 1) && c.category == 'Plausibility'
    ).length

    const plausibilityTotal = items.filter(c => c.category == 'Plausibility').length

    const plausibilityPercentPass =
      plausibilityTotal == 0 ? '-' : Math.round((plausibilityPass / plausibilityTotal) * 100) + '%'
    ...
    const conformanceFail = items.filter(
      c => (c.hasOwnProperty('passed') ? c.failed == 1 || c.isError == 1 : c.failed == 1) && c.category == 'Conformance'
    ).length

    const conformanceTotal = items.filter(c => c.category == 'Conformance').length

    const conformancePercentPass =
      conformanceTotal == 0 ? '-' : Math.round((conformancePass / conformanceTotal) * 100) + '%'
    ...
    const completenessFail = items.filter(
      c =>
        (c.hasOwnProperty('passed') ? c.failed == 1 || c.isError == 1 : c.failed == 1) && c.category == 'Completeness'
    ).length

    const completenessTotal = items.filter(c => c.category == 'Completeness').length

    const completenessPercentPass =
      completenessTotal == 0 ? '-' : Math.round((completenessPass / completenessTotal) * 100) + '%'
