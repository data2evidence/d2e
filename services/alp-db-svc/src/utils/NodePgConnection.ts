...
export class NodePgConnection implements ConnectionInterface {
  private constructor(public conn: Client, public dialect = "POSTGRES") {}

  public static createConnection(client: Client, callback: CallBackInterface) {
    try {
  ...
  public async execute(
    ...
    try {
      ...
      this.conn.query(temp, flattenParameter(parameters), (err, result) => {
        if (err) {
          logger.error(err);
  ...
  private parseSql(temp: string) {
    ...
    temp = temp.replace(/FROM SCHEMAS/gi, "FROM information_schema.schemata");
