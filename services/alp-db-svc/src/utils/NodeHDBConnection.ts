...
export class NodeHDBConnection implements ConnectionInterface {
   ...
   * @param  {any}    client
   */
  constructor(public conn: any, public dialect = "hana") {}
  ...
  public setAutoCommitToFalse(callback?: CallBackInterface) {
    try {
      ...
      if (callback) callback(null, "Set Autocommit to false successfully");
    } catch (err) {
      if (callback) {
        callback(err, null);
      } else {
    ...
    try {
      ...
      if (systemTimeAsOf == "") {
        var d = new Date(),
          ...
          day = "" + (d.getDate() + 1),
          year = d.getFullYear();
        query = `SET 'TEMPORAL_SYSTEM_TIME_AS_OF' = '${[year, month, day].join(
          "-"
        )}'`;
      } else query = `SET 'TEMPORAL_SYSTEM_TIME_AS_OF' = ${systemTimeAsOf}`;
