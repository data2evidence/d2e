...
class MultiLogs {
  ...
  constructor(genericLog: string, auditLog: object, isError: boolean = false) {
...
export const passwordReplacementRegex = /password=\S+/i;

export const enablePasswordMask = () => {
  process.stderr.write = (data: string | Buffer, cb?: any): boolean => {
    let msg = data.toString().replace(passwordReplacementRegex, "***");
