...
describe("translateHanaToDuckdb where hana and duckdb have same syntax", () => {
  it("should ", () => {
    const input = ``;
    const expected = ``;
    expect(translateHanaToDuckdb(input, "schema")).toBe(expected);
  });
  ...
  it("should reduce extra whitespace to a single space", () => {
    const input1 = ` `;
    const expected1 = ` `;
    expect(translateHanaToDuckdb(input1, "schema")).toBe(expected1);
    const input2 = `   `;
    const expected2 = ` `;
    expect(translateHanaToDuckdb(input2, "schema")).toBe(expected2);
  });
});
