...
export async function handler(
  ...
  console.log(
    "FHIR QuestionnaireResponse resource successfully inserted into OMOP GDM tables"
  );
  duckdb.close();
  return true;
}
...
async function handleItems(
  ...
  for (var item of items) {
    let itemId = uuid();
    await duckDBConn.executeQuery(
      `insert into gdm_item ("id", "gdm_questionnaire_response_id", "link_id", "text", "definition", "etl_started_at") values('${itemId}', '${questionnaireResponseId}', '${item.linkId}', '${item.text}', '${item.definition}', now())`
    );
...
async function handleAnswers(
  ...
  for (var answer of answers) {
    ...
    if (answer.valueBoolean) {
      ...
      answer.valueAttachment !== undefined &&
      answer.valueAttachment.contentType != ""
    ) {
    ...
    await duckDBConn.executeQuery(`INSERT INTO gdm_answer
      ...
      VALUES('${answerId}', '${itemId}', '${answerType}', '${answerValue}', '${
      answer.valueAttachment !== undefined
        ? answer.valueAttachment.contentType
        : ""
    }', '${
      answer.valueAttachment !== undefined
        ? answer.valueAttachment.language
        : ""
    }', '${
      answer.valueAttachment !== undefined ? answer.valueAttachment.data : ""
    }', '${
      answer.valueAttachment !== undefined ? answer.valueAttachment.url : ""
    }', '${
      answer.valueAttachment !== undefined ? answer.valueAttachment.size : ""
    }', '${
      answer.valueAttachment !== undefined ? answer.valueAttachment.hash : ""
    }', '${
      answer.valueAttachment !== undefined ? answer.valueAttachment.title : ""
    }','${
      answer.valueAttachment !== undefined
        ? answer.valueAttachment.creation
        : "1900-01-01 00:00:00"
    }', '${
      answer.valueCoding !== undefined ? answer.valueCoding.system : ""
    }', '${
      answer.valueCoding !== undefined ? answer.valueCoding.version : ""
    }', '${
      answer.valueCoding !== undefined ? answer.valueCoding.code : ""
    }', '${
      answer.valueCoding !== undefined ? answer.valueCoding.display : ""
    }', '${
      answer.valueCoding !== undefined ? answer.valueCoding.userSelected : ""
    }', '${
      answer.valueQuantity !== undefined ? answer.valueQuantity.value : ""
    }', '${
      answer.valueQuantity !== undefined ? answer.valueQuantity.comparator : ""
    }', '${
      answer.valueQuantity !== undefined ? answer.valueQuantity.unit : ""
    }', '${
      answer.valueQuantity !== undefined ? answer.valueQuantity.system : ""
    }', '${
      answer.valueQuantity !== undefined ? answer.valueQuantity.code : ""
    }', '${
      answer.valueReference !== undefined ? answer.valueReference.reference : ""
    }', '${
      answer.valueReference !== undefined ? answer.valueReference.type : ""
    }', '${
      answer.valueReference !== undefined
        ? answer.valueReference.identifier
        : ""
    }', '${
      answer.valueReference !== undefined ? answer.valueReference.display : ""
    }', now())`);
    return;
  }
}
