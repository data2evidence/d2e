...
const alpAuditLogger = Logger.CreateLogger("mri");
...
export class AuditLogger {
    ...
    public setConnection(connection: ConnectionInterface) {
    ...
    private async isEnabled() {
        return new Promise((resolve, reject) => {
            ...
            this.connection.executeQuery(
                `SELECT "Data" as DATA FROM "ConfigDbModels_Config"
                                        WHERE "Id" = ? and "Version" = ?`,
                [{ value: "audit.configId" }, { value: "2" }],
                (err, results) => {
                    if (err) {
                        return reject(err);
                    }
                    ...
                    if (results.length === 0) {
                        return reject(
                            new Error("Invalid auditlog configuration")
                        );
                    }
    ...
    public async writeFineGrained(
        ...
        try {
            const isLoggingEnabled = await this.isEnabled();
