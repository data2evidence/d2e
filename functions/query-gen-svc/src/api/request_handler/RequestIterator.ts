...
export class RequestIterator {
    constructor(private request, private config) {}
    requestWalkFunction = getJsonWalkFunction(this.request);

    get(pathExpression) {
        let results = this.requestWalkFunction(pathExpression);
        for (let i = 0, len = results.length; i < len; i++) {
            ...
            if (_isExcludePath(results[i].path)) {
                ...
                results[i].configValue = getConfigElement(
                    this.config,
                    results[i].path
                );
            }

            //add parentInteractionID which will be added to Interaction
            if (results[i].obj.attributes) {
                if (results[i].obj.attributes.parentInteraction) {
                    results[i].parentInteraction =
                        results[i].obj.attributes.parentInteraction[0];
                }
            }
