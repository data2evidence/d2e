...
export class DatasetCommandService {
  ...
  async createDatasetSnapshot(snapshotDto: IDatasetSnapshotDto) {
    const createSnapshotFn = async (
      ...
      const sourceDatasetDetail = await this.detailRepo.getDetail(
        sourceDatasetId
      );
      ...
      const sourceDatasetAttributes = await this.attributeRepo.getAttributeDto(
        sourceDatasetId
      );
  ...
  private async updateDatasetTags(
    ...
    if (tagChanges.deletedTags) {
      this.logger.debug(
        `Delete dataset ${datasetId} tags: ${JSON.stringify(
          tagChanges.deletedTags
        )}`
      );
  ...
  private async filterAttributeChanges(
    ...
    const currDatasetAttributes = await this.attributeRepo.getAttributeDto(
      datasetId
    );
