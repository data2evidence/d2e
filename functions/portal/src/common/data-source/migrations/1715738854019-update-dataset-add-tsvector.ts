...
export class UpdateDatasetAddTsvector1715738854019
  implements MigrationInterface
{
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "portal"."dataset_detail" DROP COLUMN "search_tsv"`
    );
    await queryRunner.query(
      `ALTER TABLE "portal"."dataset_attribute" DROP COLUMN "search_tsv"`
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "portal"."dataset_detail_search_tsv_index"`
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "portal"."dataset_attribute_search_tsv_index"`
    );
  }
}
