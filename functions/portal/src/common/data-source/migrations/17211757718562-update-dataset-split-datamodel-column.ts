...
export class UpdateDatasetSplitDatamodelColumn17211757718562
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `UPDATE portal.dataset SET plugin = SPLIT_PART(data_model, ' ', 2)`
    );
    await queryRunner.query(
      `UPDATE portal.dataset SET plugin = replace(replace(plugin, '[', ''), ']', '')`
    );
    await queryRunner.query(
      `UPDATE portal.dataset SET data_model = SPLIT_PART(data_model, ' ', 1)`
    );
  }
