...
export class UpdateMetadataConfigRelationship1701165537531
  implements MigrationInterface
{
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "portal"."dataset_tag" DROP CONSTRAINT "FK_8304fa2239f6a7592ef3876e1f5"`
    );
    await queryRunner.query(
      `ALTER TABLE "portal"."dataset_attribute" ALTER COLUMN "id" SET DEFAULT nextval('portal.dataset_metadata_id_seq')`
    );
    await queryRunner.query(
      `ALTER TABLE "portal"."dataset_attribute" ALTER COLUMN "id" DROP DEFAULT`
    );
    await queryRunner.query(
      `DROP SEQUENCE "portal"."dataset_attribute_id_seq"`
    );
  }
}
