import { Body, Controller, Delete, HTTP_STATUS, HttpException, Middleware, Param, Post } from '@danet/core'
...
@Controller("user-artifact/group")
export class GroupController {
  ...
  async addUserToGroup(@Body('userId') userId: string) {
    ...
    if (userExists) {
      throw new HttpException(HTTP_STATUS.CONFLICT, 'User already exists in the group')
    }
