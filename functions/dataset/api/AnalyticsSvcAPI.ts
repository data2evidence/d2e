...
export class AnalyticsSvcAPI {
  ...
  async getCdmSchemaSnapshotMetadata(
    databaseDialect: string,
    databaseCode: string,
    schemaName: string
  ) {
    this.logger.info(
      `Getting CDM schema snapshot metadata for ${schemaName} in ${databaseCode}`
    );
    const options = await this.getRequestConfig();
    ...
    throw new Error(
      `Failed to get CDM schema snapshot metadata for ${schemaName} in ${databaseCode}`
    );
  }
}
