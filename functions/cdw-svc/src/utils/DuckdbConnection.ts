import { Connection as c } from "@alp/alp-base-utils";
...
export class DuckdbConnection implements ConnectionInterface {
  ...
  public static async createConnection(callback) {
    try {
      let dbPathString = env.DUCKDB_PATH;
      if (fs.existsSync(dbPathString)) {
        const duckdDB = new duckdb.Database(
          `${dbPathString}/${getDefaultSchemaName()}`,
          {
            access_mode: "READONLY",
          }
        );
        const duckdDBconn = duckdDB.connect();
      ...
      const duckdDB = await Database.create(duckdbFilePath, OPEN_READONLY);
  ...
  public async execute(
    ...
    try {
      ...
      const result = await this.conn.all(temp, ...flattenParameter(parameters));
