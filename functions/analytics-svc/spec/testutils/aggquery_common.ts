...
export async function aggquery_setup(
  done: (client, testEnvironment, patientCreator) => void
) {
  let testEnvironment;
  ...
  await dbConnectionUtil.DBConnectionUtil.getDbClient(credentials, (err, c) => {
    ...
    let initTestEnvironment = (cb) => {
      testEnvironment = new TestEnvironment(
        ...
        (err, results) => {
          ...
          testsLogger("initTestEnvironment ...");
          cb(null);
        }
      );
    };
    ...
    let initPatientCreator = (cb) => {
      patientCreator = new PatientCreator(
        ...
        (err, results) => {
          ...
          testsLogger("initPatientCreator ...");
          cb(null);
        }
      );
    };
    ...
    async.series(
      ...
      (err, data) => {
        ...
        testsLogger("aggquery_common done ...");
        done(connection, testEnvironment, patientCreator);
      }
    );
  });
}
