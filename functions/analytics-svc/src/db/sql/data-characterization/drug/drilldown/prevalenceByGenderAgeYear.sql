...
SELECT
  ...
  cast(CAST(CASE WHEN num_analysis_id = 704 THEN num_stratum_4 ELSE null END AS INT) * 10 AS VARCHAR(11)) || '-' || cast((CAST(CASE WHEN num_analysis_id = 704 THEN num_stratum_4 ELSE null END AS INT) + 1) * 10 - 1 AS VARCHAR(11)) AS trellis_name,
  ...
  ROUND(1000 * (1.0 * num_count_value / denom_count_value), 5) AS y_prevalence_1000_pp
FROM (
        SELECT num.analysis_id    AS num_analysis_id,
