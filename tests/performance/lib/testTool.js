...
Test_tool.prototype.call1 = function (client, params, callback) {
  ...
  client.connect(function (err) {
    ...
    that.create_schema(client, function (body) {
      ...
      client.prepare(
        'SELECT TIMESTAMP FROM PERFORMANCE.MAIN WHERE FILE = ? ORDER BY TIMESTAMP DESC LIMIT 1',
        function (err, statement) {
          if (err) {
            return console.error('Prepare error:', err)
          }
          statement.exec([[fileName]], function (err, affectedRows) {
            if (err) {
...
Test_tool.prototype.call2 = function (client, fileName, callback) {
  ...
  client.prepare(query, function (err, statement) {
    ...
    statement.exec([[fileName, time.toString()]], function (err, affectedRows) {
      var q = 0
        ...
        if (q < affectedRows.length) {
          ...
          that.call3(url[q], headers[q], parameter[q], payload[q], q, method, function (message, body) {
            if (message === 'success') {
              ...
              var urlParts = url[q].match(
                /^(?:((?:https?|s?ftp):)\/\/)([^:\/\s]+)(?::(\d*))?(?:\/([^\s?#]+)?([?][^?#]*)?(#.*)?)?/
              )
              var printURL = urlParts[4]
              if (urlParts[5]) printURL += urlParts[5]
              if (printURL.length > 100) {
                printURL = printURL.substr(0, 100) + '...'
              }
              values.push([
                printURL,
                result1[q][0] + '--' + result2[q][0],
                result1[q][1] + '--' + result2[q][1],
                result1[q][2] + '--' + result2[q][2],
                result1[q][3] + '--' + result2[q][3],
                result1[q][4] + '--' + result2[q][4]
              ])
    ...
    that.write_xml_report(rows, size, old_result, new_result, function (body) {
      ...
      console.table(
        [
          ...
          'Cached End2End(Old/New)',
          'Non Cached End2End(Old/New)',
          'Database Processing(Old/New)',
          'SD Cached(Old/New)',
          'SD No Cached(Old/New)'
        ],
        values
      )
...
Test_tool.prototype.write_xml = function (rows, size, old_result, new_result, call, cb) {
        ...
        el('testsuite', function (el, at) {
          ...
          for (var x = 0; x < size; x++) {
            ...
            if (new_result[x][5] !== 200 && new_result[x][5] !== 202) {
              ...
              that.max_cached_dev !== 0 &&
              that.max_cached === 0 &&
              that.max_nocached === 0 &&
              that.max_nocached_dev === 0
            ) {
          ...
          for (var x = 0; x < size; x++) {
            ...
            el('testcase', function (el, at) {
              ...
              var actualUrl =
                '80' + that.instance + actualUrlTemp.substring(actualUrlTemp.indexOf('/'), actualUrlTemp.length)
              ...
              el(
                ...
                'Request URL and parameters: ' +
                  actualUrl +
                  '\n Payload: ' +
                  rows[x].PAYLOAD.toString().replace(/[`<>\\\\\/]/gi, '') +
                  '\n Cached End2End(Old): ' +
                  rows[x].AVG_TIME_CACHED / 1000 +
                  '\n Cached End2End(New):' +
                  new_result[x][0] +
                  ' \n Non Cached End2End(Old): ' +
                  rows[x].AVG_TIME_NOCACHED / 1000 +
                  ' \n Non Cached End2End(New):' +
                  new_result[x][1] +
                  '\n Database Processing(Old): ' +
                  rows[x].AVG_DATABASE_PROCESSING_TIME / 1000000 +
  ...
  fs.writeFile(
    __dirname +
      ...
      folderName +
      '_' +
      that.params[2].substring(that.params[2].lastIndexOf('/') + 1, that.params[2].indexOf('.')) +
      '.xml',
    data + '\n',
    'UTF-8',
    function (err) {
  ...
  ;(function hdb_loop_nested_exec() {
    if (attempt < maxRequests) {
      ...
      var sql = mysql.format(
        "select TOP 8 STATEMENT_STRING, AVG_EXECUTION_TIME, EXECUTION_COUNT, MAX_EXECUTION_TIME, MAX_PREPARATION_TIME, PREPARATION_COUNT, TOTAL_EXECUTION_TIME, TOTAL_PREPARATION_TIME, AVG_PREPARATION_TIME from M_SQL_PLAN_CACHE where USER_NAME = 'HPH_TECHNICAL_USER' order by TOTAL_EXECUTION_TIME desc"
      )
      var myHanaRequest = new HanaRequest(
        getMriAdminUserLogin(that.host, that.instance, that.user_http, that.password_http)
      )
      ...
      client2.connect(function (err) {
        ...
        myHanaRequest.request(setQuery, function (err, response, body) {
          ...
          else console.log('request successful: ' + response.statusCode + '\r\nResponse body: ' + responseBody)
      ...
      return callback(
        response_codes,
        average_total_time_cached,
        average_total_time_nocached,
        average_total_time,
        queries
      )
    }
