...
var walkSpecsSync = function( dir ) {
    ...
    list.forEach( function( file ) {
        ...
        if ( stat && stat.isDirectory() )
            result = result.concat( walkSpecsSync( file ) );
        else if ( filePatternRegex.test( file ) )
            result.push( file );
    } );
    return result;
};
...
(
    function loop_first_execution() {
        ...
        if( c <specs.length ) {
            console.log ( '\r\n---> Run test "' + specs[c] + '":\r\n' );
            var parameters = [ '', '', specs[c], accessConfig.host, accessConfig.instance,accessConfig.user_hdb, accessConfig.password_hdb ];
