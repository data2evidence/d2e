...
function replaceNamedAliases(sqlStr, aliases) {
    ...
    var aliasRegExp = new RegExp(' \(' + aliases.join('|') + '\)\( |.|,|$\)',
    'g');
    ...
    var processedString = applyToUnquotedParts(sqlStr, function (argStr) {
        var processedArgStr = argStr.replace(aliasRegExp, function (fullMatch,
            capture1, capture2) {
            var result;
...
function findAliases(sqlStr) {
        ...
        // alias.
    var aliasRegExp = /(?: AS (\w+)(?: |,|$))|(?:(?:FROM|(?:JOIN(?: \\\()?)) (?:(?:\")?\w+(?:\")?\\\.)?(?:\")?(?:\w+|(?:\w+\\\.)*\w+::(?:\w+\\\.)*\w+)(?:\")?(?! (?:WHERE|GROUP|HAVING)) (\w+)(?:$| ))/g;
    var match;
    var aliases = [];
