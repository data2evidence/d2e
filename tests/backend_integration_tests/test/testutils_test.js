    ...
    describe('TEST UTILITIES TEST SUITE', function () {
        ...
        describe('normalizeSql()... ', function () {
            ...
            it(' returns a capitalized SQL string when the argument does not contain quotations marks', function () {
                expect(testUtilLib.normalizeSql('select name from myschema.mytable'))
                    .to.equal('SELECT NAME FROM MYSCHEMA.MYTABLE');
                expect(testUtilLib.normalizeSql('SELECT NAME FROM MYSCHEMA.MYTABLE'))
                    .to.equal('SELECT NAME FROM MYSCHEMA.MYTABLE');
                expect(testUtilLib.normalizeSql('SELECT NaMe FROM MYschema.MYTABLE'))
                    .to.equal('SELECT NAME FROM MYSCHEMA.MYTABLE');
            ...
            it(' puts a single space on each side of parentheses', function () {
                expect(testUtilLib.normalizeSql('SELECT NAME FROM MYSCHEMA.MYTABLE WHERE ((A > 1  ) AND (B < 2) )  )'))
            ...
            it(' does not change the capitalization of strings inside double quotes', function () {
                expect(testUtilLib.normalizeSql('SELECT NAME FROM "MySchema"."mytable" WHERE A > 1'))
                    .to.equal('SELECT NAME FROM "MySchema"."mytable" WHERE A > 1');
            });
            ...
            it(' does not change the capitalization of strings inside single quotes', function () {
                expect(testUtilLib.normalizeSql('SELECT NAME FROM MYSCHEMA.MYTABLE WHERE A = \'someValue\' AND B = \'SOMEVALUE\' AND C = 1'))
            ...
            it(' correctly handles single quotes at end of string', function () {
                expect(testUtilLib.normalizeSql('SELECT NAME FROM MYSCHEMA.MYTABLE WHERE A = \'someValue\' AND B = \'SOMEvalue\''))
            ...
            it(' correctly handles a realistic query snippet', function () {
                ...
                expect(testUtilLib.normalizeSql(rawSql))
            ...
            it(' correctly handles a realistic query snippet by pattern matching', function () {
                var rawSql = '"legacy.cdw.db.models::InterfaceViews.PATIENT" p0 ';
                var expectedSql = '"legacy.cdw.db.models::InterfaceViews.PATIENT" P0';
                var expectedSqlRegExp = new RegExp(
                    expectedSql);
                expect(testUtilLib.normalizeSql(rawSql))
                    .to.match(expectedSqlRegExp);
            });

            it(' correctly handles a realistic query', function () {
                var rawSql = 'SELECT COUNT(DISTINCT(p0.PATIENT_ID)) AS pcount1, SUBSTR(c0.CODE,0,3) AS icd1 FROM "CDMDEFAULT"."legacy.cdw.db.models::InterfaceViews.PATIENT" p0 ';
                var expectedSql = 'SELECT COUNT ( DISTINCT ( P0.PATIENT_ID ) ) AS PCOUNT1, SUBSTR ( C0.CODE,0,3 ) AS ICD1 FROM "CDMDEFAULT"."legacy.cdw.db.models::InterfaceViews.PATIENT" P0';
                expect(testUtilLib.normalizeSql(rawSql))
                    .to.equal(expectedSql);
            });

            it(' correctly handles a simple Medexplorer query', function () {
                var rawSql = 'SELECT COUNT(DISTINCT(p0.PATIENT_ID)) AS pcount1, SUBSTR(c0.CODE,0,3) AS icd1' +
                    ...
                    ' GROUP BY SUBSTR(c0.CODE,0,3) ' + ' HAVING COUNT(DISTINCT(p0.PATIENT_ID)) >= \'15\'' +
                    ' ORDER BY icd1 ASC';
                var expectedSql = 'SELECT COUNT ( DISTINCT ( P0.PATIENT_ID ) ) AS PCOUNT1, SUBSTR ( C0.CODE,0,3 ) AS ICD1' +
                    ' FROM "CDMDEFAULT"."legacy.cdw.db.models::InterfaceViews.PATIENT" P0' +
                    ' LEFT JOIN ( "CDMDEFAULT"."legacy.cdw.db.models::InterfaceViews.INTERACTIONS" I0' +
                    ' LEFT JOIN "CDMDEFAULT"."legacy.cdw.db.models::InterfaceViews.INTERACTION_DETAILS_EAV" C0' +
                    ' ON I0.INTERACTION_ID = C0.INTERACTION_ID AND C0.CATALOG = \'ICD\' )' +
        ...
        describe('getSqlRegExpString()... ', function () {
            ...
            it('replaces a single table alias with a regular expression', function () {
                ...
                expect(testUtilLib.getSqlRegExpString(rawSql1))
                    .to.equal(expectedRegExpString1);
                expect(testUtilLib.normalizeSql(rawSql1))
                    .to.match( new RegExp( testUtilLib.getSqlRegExpString(rawSql1)));
                var rawSql2 = 'BLAH JOIN ( MYTABLE m1 )';
                var expectedRegExpString2 = 'BLAH JOIN \\( MYTABLE (\\\w+) \\)';
                expect(testUtilLib.getSqlRegExpString(rawSql2))
                    .to.equal(expectedRegExpString2);
                expect(testUtilLib.normalizeSql(rawSql2))
                    .to.match( new RegExp( testUtilLib.getSqlRegExpString(rawSql2)));
                var rawSql3 = 'BLAH JOIN "MYSCHEMA"."MYPACKAGE::MYTABLE" m1';
                var expectedRegExpString3 = 'BLAH JOIN "MYSCHEMA"\\."MYPACKAGE::MYTABLE" (\\\w+)';
                expect(testUtilLib.getSqlRegExpString(rawSql3))
                    .to.equal(expectedRegExpString3);
                expect(testUtilLib.normalizeSql(rawSql3))
                    .to.match( new RegExp( testUtilLib.getSqlRegExpString(rawSql3)));
                var rawSql4 = 'BLAH JOIN "MYSCHEMA"."MY.PACKAGE::MYTABLE" m1';
                var expectedRegExpString4 = 'BLAH JOIN "MYSCHEMA"\\."MY\\.PACKAGE::MYTABLE" (\\\w+)';
                expect(testUtilLib.getSqlRegExpString(rawSql4))
                    .to.equal(expectedRegExpString4);
                expect(testUtilLib.normalizeSql(rawSql4))
                    .to.match( new RegExp( testUtilLib.getSqlRegExpString(rawSql4)));
            });
            ...
            it('replaces a multiple table aliases according to order of occurrence', function () {
                ...
                var expectedRegExpString = 'SELECT (\\\w+)\\.ID FROM "MYSCHEMA"\\."MYTABLE" \\1 JOIN "MYSCHEMA"\\."OTHERTABLE" (\\\w+) ON \\2\\.X = \\1\\.Y';
                expect(testUtilLib.getSqlRegExpString(rawSql))
                    .to.equal(expectedRegExpString);
                expect(testUtilLib.normalizeSql(rawSql))
                    .to.match( new RegExp( testUtilLib.getSqlRegExpString(rawSql)));
            });
            ...
            it('correctly orders multiple column aliases according to order of occurrence', function () {
                ...
                var expectedRegExpString = 'SELECT X AS (\\\w+), S AS (\\\w+), P AS (\\\w+) FROM MYTABLE WHERE \\3 > 1 AND \\1 > 1 AND \\2 > 1';
                expect(testUtilLib.getSqlRegExpString(rawSql))
                    .to.equal(expectedRegExpString);
                expect(testUtilLib.normalizeSql(rawSql))
                    .to.match(new RegExp(testUtilLib.getSqlRegExpString(rawSql)));
            ...
            it('handles a typical Medical Explorer query', function () {
                var rawSql = 'SELECT COUNT(DISTINCT(p0.PATIENT_ID)) AS pcount1, SUBSTR(c0.CODE,0,3) AS icd1' +
                    ...
                    ' GROUP BY SUBSTR(c0.CODE,0,3) ' +
                    ' ORDER BY icd1 ASC';
                var expectedRegExpString = 'SELECT COUNT \\( DISTINCT \\( (\\\w+)\\.PATIENT_ID \\) \\) AS (\\\w+), SUBSTR \\( (\\\w+)\\.CODE,0,3 \\) AS (\\\w+)' +
        ...
        describe('toContainSameObjectsAsImpl()... ', function () {
            ...
            it('matches two empty arrays', function () {
                ...
                expect(testUtilLib.toContainSameObjectsAsImpl( testObjectArr1, testObjectArr2))
                    .to.be.true;
            });
