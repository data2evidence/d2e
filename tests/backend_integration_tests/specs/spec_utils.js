...
function createOrg(hdbClient, schemaName, orgId, cb) {
    function createOrgItself(callback) {
        ...
        var sqlCommand = 'INSERT INTO ' + fullTableName + ' ("ValidFrom", "OrgID") VALUES (\'1950-01-02T00:00:00.000Z\', \'' + orgId + '\')';
    ...
    function createOrgAncestor(callback) {
        ...
        var sqlCommand = 'INSERT INTO ' + fullTableName + ' ("OrgID", "AncestorOrgID", "Distance") VALUES (\'' + orgId + '\', \'' + orgId + '\', 0)';
        hdbClient.exec(sqlCommand, callback);
    }
...
function persistPatientSet(loadedPatientBuilder, hostConfig, configName, finalCallback) {
    ...
    // Do the actual work here
    async.series([
        setupPatientCreatorTask,
        writeTestPatientsTask
    ], stopHdbClient);
}
...
function checkAnalyticsResult(actualResult, expectedData, accuracy) {
    ...
    expectedKeys.forEach(function (key) {
        assert(expectedData[key].length === objLength, 'Invalid expected result, the result arrays must have the same length for every key!');
    });
