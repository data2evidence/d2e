...
api.createNonValidatedMriResultParser = function (response, body) {
    ...
    data.forEach(function (dataPoint) {
        assert(Object.keys(dataPoint).length === expectedDataFieldNo, 'There should be a total of ' + expectedDataFieldNo + ' categories and fields, but a datapoint contained ' + Object.keys(dataPoint).length + 'fields!');
    });
...
api.createMriResultParser = function (response, body) {
    assert(/2\d{2}/.test(response.statusCode), 'Unexpected (non-2XX) HTTP return code: ' + response.statusCode + '\nBody:\n' + body);
    assert(typeof body.noDataReason === 'undefined' || body.noDataReason === '', 'No data returned - reason:: ' + body.noDataReason);
...
function _buildDataTree(sortCat, sortMeas, data) {
    ...
    function _partitionPoints(curData, curCat) {
        ...
        curData.forEach(function (dataPoint) {
            ...
            assert(typeof curCatVal !== 'undefined', 'A data point did not contain any sorted value for the category  ' + curCat + '!');
