...
BackendUplink.prototype.redirectQueriesToTestSchema = function (schemaName, cb) {
    ...
    function activateTestSettings(baseSettings, callback) {
        ...
        that.hanaRequest.request(setQuery, function (err, response, body) {
            ...
            if (response.statusCode !== 200) {
                return callback(new Error('redirectQueriesToTestSchema - Failed to set global settings! Response code: ' + response.statusCode + '\nBody:\n' + JSON.stringify(body)));
            }
            return callback(err);
        });
    }
...
BackendUplink.prototype._replaceSchemaName = function (previousGlobalSettings, schemaName) {
    ...
    ['tableMapping', 'guardedTableMapping'].forEach(function (key) {
        Object.keys(newGlobalSettings[key]).forEach(function (placeholder) {
            newGlobalSettings[key][placeholder] = newGlobalSettings[key][placeholder].replace(/^[^\.]+\./, escapedTestTableNameWithDot);
        });
    });
...
BackendUplink.prototype.redirectQueriesBackToStandardSchema = function (cb) {
    ...
    this.hanaRequest.request(setQuery, function (err, response, body) {
        ...
        if (response.statusCode !== 200) {
            return cb(new Error('redirectQueriesBackToStandardSchema - Failed to set global settings! Response code: ' + response.statusCode + '\nBody:\n' + JSON.stringify(body)));
        }
...
BackendUplink.prototype.addCdwConfiguration = function (config, configId, configName, cb) {
    ...
    this.hanaRequest.request(setQuery, function (err, response, body) {
        ...
        if (response.statusCode !== 200) {
            ...
            if (fnPrintError(body.validationResult.cdmConfigValidationResult.errors) || 
...
/*
 * Assign an MRI configuration to a user.
 *
 * The callback is called with the err (if any) and the generated assignment ID.
 */
BackendUplink.prototype.addMriConfigurationAssignment = function (userName, mriConfigId, cb) {
    this.addConfigurationAssignment(userName, mriConfigId, 'HC/MRI/PA', cb);
};

/*
 * Remove an MRI configuration.
 */
BackendUplink.prototype.removeMriConfigurationAssignment = function (assignmentId, cb) {
    this.removeConfigurationAssignment(assignmentId, cb);
};

/*
 * Add a new Patient Summary configuration.
 */
BackendUplink.prototype.addPatientConfiguration = function (patientConfig, patientConfigId, configName, cdwConfigId, cb) {
    var reqBody = {
        action: 'activate',
...
/*
 * Assign an Patient Summary configuration to a user.
 *
 * The callback is called with the err (if any) and the generated assignment ID.
 */
BackendUplink.prototype.addPatientConfigurationAssignment = function (userName, configId, cb) {
    this.addConfigurationAssignment(userName, configId, 'HC/HPH/PATIENT', cb);
};

/*
 * Remove an Patient Summary configuration.
 */
BackendUplink.prototype.removePatientConfigurationAssignment = function (assignmentId, cb) {
    this.removeConfigurationAssignment(assignmentId, cb);
};

/*
 * Assign an configuration to a user.
 *
 * The callback is called with the err (if any) and the generated assignment ID.
 */
BackendUplink.prototype.addConfigurationAssignment = function (userName, configId, configType, cb) {
    var reqBody = {
        action: 'createUserAssignment',
        user: userName,
        name: 'Externally created: test config ' + configId + ' to user ' + userName,
        configs: [{
    ...
    this.hanaRequest.request(setQuery, function (err, response, body) {
        ...
        if (response.statusCode !== 200) {
            that.log('Non-200 reponse! Got: ' + response.statusCode);
            that.log('Body: ' + JSON.stringify(body));
            return cb(new Error('Failed to create configuration assignment for type ' + configType + '!'));
        }
...
/*
 * Remove an configuration assignment.
 */
BackendUplink.prototype.removeConfigurationAssignment = function (assignmentId, cb) {
    var reqBody = {
    ...
    this.hanaRequest.request(setQuery, function (err, response, body) {
        ...
        if (response.statusCode !== 200) {
            that.log('Non-200 reponse! Got: ' + response.statusCode);
            that.log('Body: ' + body);
            return cb(new Error('Failed to delete configuration assignment!'));
        }
        cb(err);
    });
};
