...
function trim(x) {
    try {
        if (x instanceof Object) {
            ...
            x.forEach(function (y) { return trim(y); });
        }
    }
    catch (e) {
        throw e;
...
var PatientVisitor = (function () {
    ...
    PatientVisitor.prototype.visitPatient = function () {
        ...
        this.cards = this.visitConditions(pathId, this.patient.conditions).concat(this.visitInteractions(pathId, this.patient.interactions));
        this.configData = this.patient.configData ? this.patient.configData : { configVersion: "No config version", configId: "No config id" };
    };
    PatientVisitor.prototype.visitBasicData = function (parentPathId, basicData) {
        var _this = this;
        var pathId = parentPathId.slice();
        if (basicData.pcount) {
            this.axes.push(this.createAxis("yaxis", parentPathId, "0", pathId.concat(["attributes", "pcount"]), basicData.pcount[0]));
        }
        return {
            _configPath: parentPathId.join(DELIMITER),
            _instanceNumber: 0,
            _instanceID: parentPathId.join(DELIMITER),
            _attributes: { content: Object.keys(basicData).filter(function (e) { return e !== "pcount"; }) //handle pcount attribute as an axis elsewhere
                    .map(function (attributeName) { return _this.visitAttributes(pathId, attributeName, basicData[attributeName][0], "0"); }) }
        };
    };
    PatientVisitor.prototype.visitConditions = function (parentPathId, conditions) {
        var _this = this;
        return conditions ? Object.keys(conditions).reduce(function (filtercards, conditionName) {
            ...
            Object.keys(condition.interactions).forEach(function (interactionType) {
                ...
                Object.keys(interaction).forEach(function (instanceNumber) {
                    ...
                    var filtercard = {
                        ...
                        _attributes: { content: attributes ? Object.keys(attributes).filter(function (e) { return _this.specialAttributes.indexOf(e) === -1; })
                                .reduce(function (attributeConstraints, attributeName) {
                                    attributeConstraints.push(_this.visitAttributes(instanceID, attributeName, attributes[attributeName][0], "0"));
    ...
    PatientVisitor.prototype.visitInteractions = function (parentPathId, interactions) {
        var _this = this;
        return interactions ? Object.keys(interactions).reduce(function (filtercards, interactionName) {
    ...
    PatientVisitor.prototype.visitAttributes = function (parentPathId, attributeName, attribute, instanceNumber, isFiltercard) {
        var _this = this;
        var pathId = parentPathId.concat(["attributes", attributeName]);
        Object.keys(attribute).filter(function (e) { return e === "xaxis" || e === "yaxis"; })
            .forEach(function (f) { return _this.axes.push(_this.createAxis(f, parentPathId, instanceNumber, pathId, attribute, isFiltercard)); });
        return {
            _configPath: pathId.join(DELIMITER),
            _instanceID: instanceNumber !== "0" ? parentPathId.concat([instanceNumber, "attributes", attributeName]).join(DELIMITER) : pathId.join(DELIMITER),
    ...
    PatientVisitor.prototype.visitConstraint = function (filter) {
        ...
        if (filter) {
            constraint.content = filter.map(function (e) {
                return e.and ?
                    { content: e.and.map(function (f) { return { _operator: f.op, _value: f.value }; }) } :
                    { _operator: e.op, _value: e.value };
            });
        }
        return constraint;
    };
    PatientVisitor.prototype.createAxis = function (axisType, parentPathId, instanceNumber, pathId, attribute, isFiltercard) {
        return {
            instanceID: instanceNumber !== "0" ? parentPathId.concat([instanceNumber]).join(DELIMITER) : parentPathId.join(DELIMITER),
...
function request2IFR(request) {
    ...
    var req = request.map(function (e) { return new PatientVisitor(e.patient); });
    var ifr = {
    ...
    if (req.length > 1) {
        ...
        matchAny = req.reduce(function (matchAnyList, filter) {
            filter.cards.forEach(function (e) {
                if (matchAll.filter(function (f) { return JSON.stringify(f) === JSON.stringify(e); }).length === 0) {
                    matchAnyList.push(e);
                }
            });
            return matchAnyList;
    ...
    Object.keys(request[0]).filter(function (e) { return e !== "patient"; })
        .forEach(function (f) { ifr[f] = request[0][f]; });
    trim(ifr);
    return ifr;
}
