...
# inputs
FLOW_RUNS_URL=${FLOW_RUNS_URL:-https://localhost:41100/portal/systemadmin/jobs/deployments}
...
echo ZIP_GLOB=$ZIP_GLOB
find $ZIP_DIR -name "*${ZIP_GLOB}*.zip"
...
CMD=(yarn create-postgres-cdm-schemas alpdev_pg cdmdefault cdmvocab)
echo . INFO Launch ${CMD[@]} ...
cd $GIT_BASE_DIR
${CMD[@]}
sleep 3

echo . WAIT check job flow run $FLOW_RUNS_URL
[[ $FLOW_RUNS_URL =~ runs ]] && open $FLOW_RUNS_URL
read -p "Flow success? Press any key to continue or ctrl-C to escape" -n 1 -r

echo . INFO Launch data_load_plugin ...
