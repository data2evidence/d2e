...
cat $DOTENV_YML_IN | yq -o sh 'with_entries(select(.key|test("CADDY__ALP__PUBLIC_FQDN|GH_TOKEN|GIT_TOKEN__PLUGINS_REPO_READ|HANA__CRT|HANA__HOSTNAME|HANA__TENANT_ADMIN_PASSWORD|HANA__TENANT_ADMIN_PASSWORD_SALT|HANA__TENANT_READ_PASSWORD|HANA__TENANT_READ_PASSWORD_SALT")))' >> $DOTENV_FILE_OUT
