...
cat $DOTENV_YML_IN | yq -o sh 'with_entries(select(.key|test("CADDY__ALP__PUBLIC_FQDN|DOCKER_TREX_REPO_NAME|GH_TOKEN|GIT_TOKEN__PLUGINS_REPO_READ|HANA__CRT|HANA__HOSTNAME|HANA__TENANT_ADMIN_PASSWORD_PLAIN|HANA__TENANT_READ_PASSWORD_PLAIN")))' >> $DOTENV_FILE_OUT
